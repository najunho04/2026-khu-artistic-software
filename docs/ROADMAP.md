...
> 최종 수정: 2026-09-10 — **병합 전 점검 (Phase 1~6 전체 되짚기)** 을 돌려 문서가 정해 둔 것을 구현이 지키지 않던 자리 17곳을 고쳤습니다. 형식이 깨진 요청의 500 응답, 필수 항목 누락의 조용한 통과, 날짜 경계의 UTC 판정, 같은 자원에 동시에 들어온 요청 다섯 갈래입니다. 자세한 목록은 아래 "병합 전 점검" 절에 있습니다. **스펙 변경은 없고** 테스트는 278개에서 295개로 늘었습니다.
>
> (같은 날 앞선 수정: **미확정 3건 확정** (회원 탈퇴 cascade, 캐릭터 획득·진화 규칙, 이행률 분모). 이로써 **Phase 0 블로커가 전부 해소되고 Phase 6 이 통째로 착수 가능**해졌습니다. 남은 미구현 엔드포인트는 3개(탈퇴 1 · 캐릭터 2)입니다.)
>
...
| 테스트 코드 | **278개.** 전부 통과 · skip 0 (`./gradlew test --rerun-tasks` 로 확인, 2026-09-10). 각 작업의 첫 산출물이 테스트입니다 |
...
> 🐛 **2026-09-10 수정 — 빅루틴 `sortOrder` 가 항상 비어 있던 문제.** 컬럼에 값을 넣는 코드가 한 줄도 없어 응답이 늘 `null` 이었고, 하루에 루틴이 여러 개일 때 정렬 기준이 없었습니다(저녁 루틴이 아침 루틴보다 먼저 나왔습니다). 이제 **조회할 때 `start_time` 오름차순으로 계산**해 담습니다. 저장하지 않는 이유는 시각을 고치면 순서가 어긋나기 때문입니다(ERD 1장 설계 노트). 캘린더와 기기 동기화가 같은 규칙을 씁니다.
...
> 🐛 **2026-09-10 수정 — 저장해둔 할 일 차례가 뒤집히던 문제.** 양식에 `order` 를 매겨 저장해도 루틴을 만들 때는 배열 차례대로 1, 2, 3 이 다시 매겨져 **사용자가 정한 차례가 무시**됐습니다. 빅루틴 생성 요청의 `smallRoutines[].order` 도 마찬가지로 죽은 값이었습니다. 이제 `order` 로 줄 세운 뒤 번호를 매깁니다(`sortByRequestedOrder`). `order` 가 없는 항목은 뒤로 가며 보낸 배열 차례를 지킵니다.
...
Phase 1~6 을 합쳐 `main` 에 올리기 전에 세 가지 기준으로 되짚었습니다. **보편적인 엣지케이스**, **동시 요청에서의 DB 일치성 · 멱등성**, **문서 표준과의 일치**입니다.

### 고친 것 (전부 문서가 이미 정해 둔 것을 구현이 지키지 않던 자리)

| # | 무엇이 | 증상 | 고친 곳 |
|---|---|---|---|
| 1 | 형식이 깨진 요청 | 경로 변수 타입 불일치 · 필수 파라미터 누락 · 본문 JSON 깨짐이 **전부 500** (`API.md` 2-3 은 400) | `GlobalExceptionHandler` |
| 2 | 비밀번호 없는 로그인 | 인코더가 `null` 을 받아 **500** | `AuthenticationService` |
| 3 | 순서 변경 요청에 목록 없음 | `null` 을 다루다 **500** | `RoutineService` |
| 4 | 제목 없는 양식 저장 | DB `not null` 제약까지 내려가 **500** | `RoutineTemplateService` |
| 5 | 별칭 없는 기기 수정 | **200 인데 아무것도 안 바뀜** (`API.md` 7장은 필수) | `DeviceService` |
| 6 | 필수 항목 빠진 기기 동기화 | **200 인데 아무것도 안 바뀜.** `dates` 가 빠지면 루틴을 못 받는데 오류도 없음 | `DeviceSyncService` |
| 7 | 자녀 생년월일 "미래 날짜" 판정 | UTC 로 판단해 **한국 시각 새벽에만** 오늘 날짜가 거절됨 | `ChildService` |
| 8 | 빅루틴 수정 | 종료 시각을 시작 시각보다 앞당길 수 있었음 (만들 때는 막는 규칙) | `RoutineService` |
| 9 | 동시 기기 동기화 | 같은 완료 기록으로 `exp` 가 **두 번** 오르고 첫 캐릭터가 두 마리 | 자녀 행 잠금 |
| 10 | 동시 할 일 추가 | 같은 `sort_order` 를 잡아 **서로 다른 할 일이 같은 미션**이 됨 | 자녀 행 잠금 |
| 11 | 동시 claim | 두 기기가 같은 코드로 **둘 다 성공** | 기기 행 잠금 |
| 12 | 동시 페어링 · 자녀 등록 | 상한 검사를 둘 다 통과해 **11대 · 11명** | 자녀 · 보호자 행 잠금 |
| 13 | 탈퇴와 동기화가 겹칠 때 | 잠금 순서가 엇갈려 교착 가능 | `UserService` 잠금 순서 정렬 |
| 14 | `GET /devices/:deviceId` | 문서에 있는 `childId` 가 응답에 없었음 | `DeviceDetailResponse` 신설 |

새로 쓴 테스트: `InvalidRequestIntegrationTest`(11), `ParallelRequestIntegrationTest`(4), `ChildBirthDateBoundaryTest`(1), 기기 동기화 필수 항목(1). **278 → 295개, 실패 0 · 건너뜀 0.**

### 남은 것 (결정이 필요해 손대지 않음)

| # | 항목 | 왜 지금 못 정하나 |
|---|---|---|
| A | 대상 자체가 과거 날짜일 때의 수정 · 할 일 추가 | 문서는 "오늘 이전은 건드리지 않는다" 인데 구현은 **대상 자신은 고친다.** 막을 경우의 응답 형태가 문서에 없음 (`API.md` 9장) |
| D | 캘린더 조회 범위 | `API.md` 15장 #7 미확정. 지금은 `from`~`to` 쿼리 파라미터에 상한이 없어 몇 년치 조회가 가능. **"그 달 것만" 으로 바꾸는 안**이 논의 중 |

### 같은 날 확정된 것 (B · C · E)

| 항목 | 결정 | 문서 파급 |
|---|---|---|
| **B** `PUT .../small-routines/order` 요청 형태 | ✅ **문서대로 간다.** 최상위가 배열인 `[{smallRoutineId, order}]` 이며, 구현을 그 형태로 고쳤다. 공통 envelope 는 **응답**의 규칙이라 요청까지 감쌀 이유가 없다. 순서는 배열 차례가 아니라 `order` 값이 정하고, 저장할 때 1 · 2 · 3 으로 다시 매긴다 | `API.md` 9장 (🔺 해소). `ERD` 영향 없음 |
| **C** `PAIRING_CODE_ALREADY_USED` 409 | ✅ **현행 유지.** 이미 쓴 코드도 `PAIRING_CODE_NOT_FOUND` 404 로 나간다. 코드는 3-4 표에 남겨 두되 "현재 나가지 않음" 을 명시했다. 기기가 할 일은 어느 쪽이든 재페어링 하나뿐이라 구분할 실익이 없다 | `API.md` 3-4 · 8장. `ERD` 영향 없음 |
| **E** 자녀 응답의 `createdAt` | ✅ **문서에서 뺀다.** `CHILDREN` 에 `created_at` 컬럼을 만들지 않는다(H-004 에서 뺀 컬럼을 되살리지 않는다). 앱 화면에 자녀 등록 시각을 쓰는 자리가 없다. `deviceCount`(🔺)는 그대로 미구현 | `API.md` 6장. `ERD` 영향 없음 |
