...
### 6-2. 캐릭터 (칸반: **낮음**) — 착수 가능 (2026-09-10)

`GET /characters` · `GET /children/{childId}/characters` **조회 2개**와, **기기 동기화 시점의 획득·진화**를 함께 만듭니다.

> ✅ **확정 (2026-09-10)** — 경험치는 `POST /device-api/v1/sync` 로 **새로 완료된** 할 일 1개당 `exp` +1, `exp` 10마다 `level` +1, 최대 `level` 3. 키우던 캐릭터가 3에 닿으면 도감 `code` 오름차순으로 다음 1마리를 지급합니다. `rarity`·`weight` 는 쓰지 않습니다(ERD 5-4, API 12-1). **이것으로 0-5 가 닫혔습니다.**
...
- `level` 3 에 닿은 다음 완료가 들어오면 **도감 두 번째 캐릭터**가 생기고 첫 캐릭터는 `level` 3 · `exp` 30 에 멈춘다
