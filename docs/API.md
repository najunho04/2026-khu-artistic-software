...
>
> **현재 상태 (2026-09-09)** — 정해지지 않아서 기기 체인은 **인증 실패 시 401 상태 코드만 내보내고 본문을 비워 둡니다**(`HttpStatusEntryPoint`). 앱 체인은 위의 실패 형태를 그대로 씁니다(`CommonAuthenticationEntryPoint`). 결정이 나오면 기기 쪽에도 본문을 쓰는 진입점을 붙입니다.
>
> ⚠️ **나중에 바꾸면 디바이스 API 테스트를 전부 다시 써야 하고 기기 펌웨어도 함께 고쳐야 합니다.** 하드웨어 담당과 합의가 필요한 항목입니다.
