...
>
> 최종 수정: 2026-09-10 — **병합 전 점검 반영.** 스펙은 그대로이고, 문서가 이미 정해 둔 것을 구현이 지키지 않던 자리를 채웠습니다. 형식이 깨진 요청의 상태 코드(2-3), 로그인·기기 수정·양식 저장·기기 동기화의 필수 항목 누락(4 · 7 · 8 · 10장), 빅루틴 수정의 시각 검사(9장), 같은 자원에 동시에 들어온 요청의 처리(8 · 12장)입니다. **엔드포인트 개수와 필드는 바뀌지 않았습니다.**
>
> (같은 날 앞선 수정: **미확정 2건 확정.** `DELETE /users/me` 의 cascade 정책(4장)과 캐릭터 획득·진화 규칙(12장)을 채웠습니다. 엔드포인트 개수는 그대로이며, 그동안 구현을 막고 있던 명세 공백이 사라져 **캐릭터 2개와 탈퇴 1개가 착수 가능해졌습니다.**)
>
...
| timestamp | ISO-8601 UTC | 2026-09-01T08:30:00Z | createdAt(`GET /users/me` 전용), lastSyncedAt, serverTime |
...
> ✅ **형식이 깨진 요청도 400 입니다 (2026-09-10 확인).** 경로 변수의 타입이 맞지 않는 경우(`/children/undefined`), 필수 조회 파라미터가 빠진 경우, 본문이 JSON 으로 읽히지 않는 경우가 모두 여기에 들어갑니다. code 는 `INVALID_INPUT` 이며, 값의 이름을 알 수 있으면 `details` 에 담습니다.
>
> 이것을 500 으로 내보내면 안 되는 이유는 앱이 할 일이 달라지기 때문입니다. 500 은 "서버가 고장났다" 는 뜻이라 앱은 재시도하는데, 몇 번을 보내도 같은 결과이고 실제로 고쳐야 할 곳은 앱입니다. 그 사이 서버 쪽에는 장애 신고가 들어옵니다.
...
| `PAIRING_CODE_ALREADY_USED` | 409 | 이미 사용된 페어링 코드입니다. | 일회용. ⚠️ **현재 구현에서는 나가지 않습니다** — 아래 8장 참고 |
...
**Error**: `AUTH_INVALID_CREDENTIALS` 401 (이메일이 없을 때와 비밀번호가 틀릴 때 **같은 응답**), `INVALID_INPUT` 400 (`email` 또는 `password` 를 아예 보내지 않음)

> **값을 안 보낸 것과 틀리게 보낸 것은 다릅니다.** 앞엣것은 앱이 고쳐야 할 입력 오류라 400 이고, 여기서는 가입 여부를 확인하지 않으므로 "이 이메일은 있다" 는 사실도 새어 나가지 않습니다.
...
> ✅ **`createdAt` 을 뺐습니다 (2026-09-10 확정).** `CHILDREN` 에 `created_at` 컬럼이 **없기 때문**입니다(`ERD.md` 1장). 전에는 문서만 이 필드를 요구하고 테이블에는 담을 곳이 없는 상태였습니다. 넣으려면 컬럼 추가와 마이그레이션이 따라오는데, 앱 화면에 자녀 등록 시각을 쓰는 자리가 없어 **문서에서 빼는 쪽**을 골랐습니다. 나중에 필요해지면 그때 `ERD` 부터 고칩니다. `POST /children` 응답도 같습니다.
...
**Error**: `INVALID_INPUT` 400 (`nickname` 누락 · 빈 문자열), `DEVICE_NOT_FOUND` 404, `DEVICE_FORBIDDEN` 403

- 빈 요청을 200 으로 돌려주면 앱은 별칭이 바뀌었다고 믿는데 실제로는 아무것도 바뀌지 않아, 화면과 서버가 어긋난 채로 남습니다.
...
1. `pairingCode`로 `status = 'PENDING'`인 행을 **잠그고** 조회 (`unique(pairing_code) where status = 'PENDING'` 덕분에 단일 행 특정). 잠그지 않으면 같은 코드로 두 기기가 동시에 들어왔을 때 둘 다 신분증을 발급받고 나중 것만 저장되어, 먼저 성공한 기기가 서버에 없는 신분증을 들고 다니게 됩니다. 그 기기는 이후 모든 요청에서 인증에 실패하는데 스스로는 성공했다고 믿어 재페어링 안내도 뜨지 않습니다
2. 만료 시각 검증 (발급 후 10분)
...
**Error**: `PAIRING_CODE_NOT_FOUND` 404, `PAIRING_CODE_EXPIRED` 410, `DEVICE_UID_ALREADY_PAIRED` 409

> ✅ **이미 사용된 코드는 404 입니다 (2026-09-10 확정, 현행 유지).** claim 이 끝나면 `pairing_code` 를 비우므로(위 5단계) 서버에는 "이 코드가 전에 쓰였다" 는 흔적이 남지 않습니다. 그래서 `PAIRING_CODE_ALREADY_USED` 409 는 **던질 자리가 없고**, 이미 쓴 코드도 없는 코드와 똑같이 `PAIRING_CODE_NOT_FOUND` 404 로 나갑니다.
>
> 코드는 3-4 표에 **남겨 둡니다.** 지우면 나중에 구분이 필요해졌을 때 이름부터 다시 정해야 하고, 앱·기기 어느 쪽도 이 코드로 분기하고 있지 않습니다. 구분하려면 코드를 비우지 않고 `status` 로 판정하도록 바꿔야 하는데, **기기가 둘을 다르게 안내할 이유가 없습니다.** 어느 쪽이든 기기가 할 일은 앱에서 페어링을 다시 시작하는 것 하나입니다.
...
**Error**: `DEVICE_UNAUTHORIZED` 401 (`X-Device-Uuid` 가 없거나 유효하지 않음. 자체 복구 경로가 없으므로 기기는 재페어링을 안내해야 합니다) · `INVALID_INPUT` 400 (`battery` 가 0~100 밖, `dates` 가 상한 초과, `status` 가 `DONE`/`PENDING` 이 아님, **네 필수 항목(`battery` · `firmware` · `completions` · `dates`) 중 하나라도 빠짐**)

- ✅ **필수 항목이 빠지면 거절합니다 (2026-09-10 확정).** 빠진 채로 200 을 돌려주면 기기는 반영됐다고 믿는데 서버는 아무것도 하지 않은 상태로 갈라집니다. 특히 `dates` 가 빠지면 루틴을 한 줄도 받지 못하면서 오류도 뜨지 않아, 기기 화면이 왜 비어 있는지 알아낼 방법이 없습니다. 완료 기록이 없는 경우는 `null` 이 아니라 **빈 배열**로 보냅니다.
...
**Error**: `BIG_ROUTINE_NOT_FOUND` 404, `CHILD_FORBIDDEN` 403, `ROUTINE_INVALID_TIME_RANGE` 400

- ✅ **고치고 난 뒤의 시각 짝도 검사합니다 (2026-09-10 확정).** 만들 때 "`endTime` 은 `startTime` 보다 뒤" 를 검사하는데 고칠 때 검사하지 않으면 같은 규칙이 수정 한 번으로 깨집니다. 보내지 않은 값은 지금 저장된 값과 견줍니다. 시작 시각만 고치는 요청도 종료 시각을 지나칠 수 있기 때문입니다.
...
- 해당 빅루틴의 스몰루틴 **전체**가 배열에 포함되어야 합니다. 누락이 있으면 `ROUTINE_ORDER_MISMATCH` 400. `smallRoutineId` 가 비어 있거나 같은 id 가 두 번 오는 것도 같은 오류입니다.
- ✅ **요청 최상위가 배열인 이 형태로 확정했습니다 (2026-09-10).** 전에 여기 적혀 있던 "`{ "orders": [...] }` 로 감싸는 편이 일관적" 이라는 제안은 **채택하지 않았습니다.** 공통 envelope 는 **응답**의 규칙이고(2장), 요청 본문까지 같은 모양이어야 할 이유가 없습니다. 응답은 지금도 공통 envelope 로 감싸 나갑니다.
- **배열에 담긴 차례가 아니라 `order` 값이 순서를 정합니다.** 앱의 드래그 정렬이 화면에서 옮긴 결과를 숫자로 적어 보내기 때문입니다. 서버는 그 값으로 줄을 세운 뒤 `sortOrder` 를 1, 2, 3 으로 **다시 매겨 저장**합니다. 만들 때와 같은 규칙이며(9장 `order` 의 뜻), 보낸 숫자를 그대로 저장하지 않는 이유도 같습니다. `order` 를 적지 않은 항목은 뒤로 가며 보낸 배열 차례를 지킵니다.
- `series_id` 는 **건드리지 않습니다.** 순서를 바꾼 것이지 다른 미션이 된 것이 아니므로, 미션별 이행률은 그대로 이어집니다.
...
**Error**: `INVALID_INPUT` 400 (`title` 누락 · 빈 문자열), `CHILD_NOT_FOUND` 404, `CHILD_FORBIDDEN` 403 (1-1-1)

- `title` 은 `ROUTINE_TEMPLATES.title` 이 `not null` 이라 없으면 DB 까지 내려가 제약 위반이 됩니다. 그 전에 400 으로 돌려줘야 앱이 무엇을 고쳐야 하는지 알 수 있고, 제목 없이는 목록에서 고를 수도 없습니다.
...
- ✅ **같은 자녀의 동기화는 한 줄로 세웁니다 (2026-09-10 확정).** 기기는 응답을 못 받으면 같은 요청을 다시 보내는데, 앞의 요청이 죽은 것이 아니라 늦게 도착하는 중일 수 있습니다. 두 요청이 겹치면 둘 다 "이번에 처음 완료됐다" 로 세어 경험치가 두 번 오르고, 보유 캐릭터가 없던 자녀에게는 첫 캐릭터가 두 마리 지급됩니다. 자녀 행을 잠가(다른 요청을 앞의 것이 끝날 때까지 기다리게 해) 그 틈을 없앱니다. 자녀 한 명에 걸린 요청만 기다리므로 다른 자녀의 동기화는 그대로 지나갑니다.
