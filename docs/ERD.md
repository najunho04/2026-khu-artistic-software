...
- **USERS** — `provider`, `provider_user_id`는 각각 소셜 로그인 제공자와 소셜 로그인 uid.
  소셜 로그인은 **구글로 제한**하기로 확정(2026-09-04)되어 현재 `provider` 에 들어가는 값은 `GOOGLE` 하나뿐입니다. 그래도 컬럼과 `unique(provider, provider_user_id)` 제약은 **그대로 둡니다.** 제공자가 하나뿐이라고 컬럼을 없애면 나중에 다른 제공자를 붙일 때 이미 쌓인 데이터를 마이그레이션해야 하기 때문입니다. 값 목록을 DB `check` 제약으로 묶지 않는 것도 같은 이유입니다.
