...
> 최종 수정: 2026-09-10 — **미확정 2건 확정**(회원 탈퇴 cascade, 캐릭터 획득·진화 규칙). 테이블·컬럼은 바뀌지 않았고 5장 이슈와 6장 미확정 목록만 갱신했습니다. 같은 날 확정한 이행률 분모 규칙은 이 문서에 이미 반영돼 있어 **영향 없음**입니다.
...
| 최대 레벨 | **3.** `exp` 는 30에서 멈추고 더 쌓지 않습니다 |
| 새 캐릭터 획득 | 키우던 캐릭터가 `level` 3 에 도달하면 **도감(`CHARACTERS`)의 `code` 오름차순으로 아직 없는 다음 캐릭터 1마리**를 지급합니다 |
...
자녀는 언제나 **`level` 3 미만인 캐릭터를 최대 한 마리만** 가집니다. 그 한 마리가 "지금 키우는 캐릭터"이고, 경험치는 항상 그 마리에만 들어갑니다. 여러 마리에 나눠 넣지 않는 이유는, 나눠 넣으면 "어느 마리가 자라는 중인지"를 따로 저장할 컬럼이 필요해지는데 `CHILD_CHARACTERS` 에 그런 컬럼이 없기 때문입니다. 지금 규칙은 `level < 3` 인 행 하나를 찾는 것만으로 같은 답을 얻습니다.
